...
public class JobController {
	...
	public ResponseEntity<?> saveJob(@RequestBody Job job, HttpSession session) {
		...
		if (email == null) {// not logged in
			...
			return new ResponseEntity<ErrorClazz>(errorClazz, HttpStatus.UNAUTHORIZED);// 2nd
																						// callback
																						// fun
		}
		...
		try {
			...
			return new ResponseEntity<ErrorClazz>(errorClazz, HttpStatus.INTERNAL_SERVER_ERROR);// 2nd
																								// callback
																								// fun
		}
	}

	@RequestMapping(value = "/activejobs", method = RequestMethod.GET) // select
																		// *
																		// from
																		// job
																		// where
	...
	@RequestMapping(value = "/inactivejobs", method = RequestMethod.GET) // select
																			// *
																			// from
																			// job
																			// where
																			// active=false
	public ResponseEntity<?> getInActiveJobs(HttpSession session) {// Authenctication
																	// and
																	// authorization
		String email = (String) session.getAttribute("email");
		if (email == null) {
	...
	public ResponseEntity<?> updateJob(HttpSession session, @RequestBody Job job) {// job
																					// with
																					// updated
																					// active
																					// value
